...
(function(){
  ...
  req('/.wormkey/me').then(function(r){ if (!r.ok) return; return r.json(); }).then(function(me){
    ...
    root.style.cssText = 'position:fixed;bottom:max(10px,env(safe-area-inset-bottom,10px));left:50%;transform:translateX(-50%);z-index:2147483647;display:flex;flex-direction:column;justify-content:flex-end;gap:4px;width:fit-content;min-width:0;max-width:calc(100vw - 20px);user-select:none';
    var dragging = false, dx = 0, dy = 0;
    function normEv(ev){ return ev.touches ? ev.touches[0] : ev; }
    ...
    function onEnd(ev){
      var e = (ev.changedTouches && ev.changedTouches[0]) ? ev.changedTouches[0] : ev;
      if (dragging && (Math.abs(e.clientX - wormkeyDownX) > 4 || Math.abs(e.clientY - wormkeyDownY) > 4) === false) {
        panelOpen = false;
    ...
    function row(label, value, onCopy){
      ...
      if (onCopy && value) {
        ...
        copyBtn.className = 'wormkey-copy-btn';
    ...
    var prevViewerCount = 0;
    function addLog(msg){ logEntries.push(msg); var el = document.createElement('div'); el.textContent = msg; el.style.marginBottom = '2px'; logsList.appendChild(el); logsList.scrollTop = logsList.scrollHeight; }

    var bar = document.createElement('div');
    bar.className = 'wormkey-bar';
    ...
    wormkeyHandle.onmousedown = function(ev){ startDrag(ev); wormkeyHandle.style.cursor = 'grabbing'; };
    wormkeyHandle.ontouchstart = function(ev){ ev.preventDefault(); startDrag(ev); wormkeyHandle.style.cursor = 'grabbing'; };
    ...
    logsTabBtn.style.cssText = 'border:0;background:transparent;border-radius:6px;padding:0 10px;cursor:pointer;color:#fff;font:10px "Geist",sans-serif;font-weight:500;opacity:0.5;white-space:nowrap;display:flex;align-items:center;justify-content:center;transition:background .15s';
    logsTabBtn.onclick = function(){ activeTab = 'logs'; panelOpen = true; panel.style.display = 'flex'; setTab(); };
    bar.appendChild(logsTabBtn);

    var viewsTabBtn = document.createElement('button');
    viewsTabBtn.className = 'tabbar-btn';
    viewsTabBtn.textContent = 'Views';
    viewsTabBtn.style.cssText = 'border:0;background:transparent;border-radius:6px;padding:0 10px;cursor:pointer;color:#fff;font:10px "Geist",sans-serif;font-weight:500;opacity:0.5;white-space:nowrap;display:flex;align-items:center;justify-content:center;transition:background .15s';
    viewsTabBtn.onclick = function(){ activeTab = 'views'; panelOpen = true; panel.style.display = 'flex'; setTab(); };
    bar.appendChild(viewsTabBtn);

    var closeBtn = document.createElement('button');
    closeBtn.className = 'tabbar-btn';
    closeBtn.textContent = 'Close Tunnel';
    closeBtn.style.cssText = 'border:0;background:transparent;border-radius:6px;padding:0 10px;cursor:pointer;color:#fff;font:10px "Geist",sans-serif;font-weight:500;opacity:0.5;white-space:nowrap;display:flex;align-items:center;justify-content:center;transition:background .15s';
    ...
    statusWrap.className = 'tabbar-connected tabbar-btn';
    statusWrap.style.cssText = 'display:flex;gap:6px;align-items:center;padding:0 10px;cursor:pointer;border-radius:6px;transition:background .15s';
    statusWrap.title = 'Click to view logs';
    ...
    statusWrap.onclick = function(){ activeTab = 'logs'; panelOpen = true; panel.style.display = 'flex'; setTab(); };
    bar.appendChild(statusWrap);

    var viewerWrap = document.createElement('div');
    viewerWrap.className = 'tabbar-views tabbar-btn';
    viewerWrap.style.cssText = 'display:flex;gap:6px;align-items:center;padding:0 10px;cursor:pointer;border-radius:6px;transition:background .15s';
    viewerWrap.title = 'Click to view viewers';
    ...
    viewerWrap.onclick = function(){ activeTab = 'views'; panelOpen = true; panel.style.display = 'flex'; setTab(); };
    bar.appendChild(viewerWrap);

    var mq = window.matchMedia('(max-width:480px)');
    function setMobileLabels(){
      var mobile = mq.matches;
      copyTabBtn.textContent = mobile ? 'Copy' : 'Copy Url';
      closeBtn.textContent = mobile ? 'Close' : 'Close Tunnel';
    }
    mq.addEventListener('change', setMobileLabels);
    setMobileLabels();
    ...
    function refresh(){
      ...
      req('/.wormkey/state').then(function(r){ if (!r.ok) throw new Error('state'); return r.json(); }).then(function(s){
        var count = s.activeViewers || 0;
        viewerCount.textContent = String(count);
        if (connectedAt === null) {
          connectedAt = new Date();
          addLog('Connected at ' + connectedAt.toLocaleTimeString());
        }
        if (count !== prevViewerCount) {
          if (count > prevViewerCount) addLog('Viewer joined (' + count + ' total)');
          else if (count < prevViewerCount && prevViewerCount > 0) addLog('Viewer left (' + count + ' total)');
          prevViewerCount = count;
        }
        viewsList.innerHTML = '';
        ...
        if (connectedAt) addLog('Disconnected at ' + new Date().toLocaleTimeString());
      });
    ...
    closeBtn.onclick = function(){
      ...
      req('/.wormkey/close', { method:'POST' }).then(function(){
        tunnelClosed = true;
        closeBtn.textContent = 'Open Tunnel';
        closeBtn.title = 'Reload page after running wormkey again';
        statusText.textContent = 'Closed';
        statusText.style.color = '#818181';
        statusWrap.querySelectorAll('svg path').forEach(function(p){ p.setAttribute('fill', '#818181'); });
        addLog('Tunnel closed at ' + new Date().toLocaleTimeString());
      });
    };

    panel.appendChild(copyContent);
    panel.appendChild(logsContent);
    panel.appendChild(viewsContent);
